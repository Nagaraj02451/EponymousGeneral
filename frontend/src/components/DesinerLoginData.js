...
// import Header from '../layouts/Header';
// import Footer from '../layouts/Footer';
export default function DesinerLoginData({ product }) {
  ...
  const { id } = useParams()
  console.log(product.images.length, "ghfchv");
  const datas = [
    {
      title: "Profile",
      des: "one",
  ...
  return (
    <>
      ...
      {loading ? <Loader /> :
        ...
        <div className='designerdashboardinne2'>
        
        <Link to={`/designer/${product._id}`} className='yuiguio'><div >
           EDIT LOOKBOOK
           </div></Link>  
